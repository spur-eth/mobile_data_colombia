...
def filter_pings_for_visit_approx(
    df: pd.DataFrame, time_window_minutes: int = 20
) -> pd.DataFrame:
    ...
    def filter_user_pings(user_df):
        ...
        keep_pings = (
            user_df["time_diff"] > pd.Timedelta(minutes=time_window_minutes)
        ) | user_df["time_diff"].isna()
